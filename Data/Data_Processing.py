...
class Data_Processing():
    ...
    def get_neighbor_triplets(game: pgn.Game) -> List[Tuple]:
        #Parameters:
        #chess game

        #Returns
        #Returns a list of Tuples which contain
        #   -a int representing the move 1-white / -1-black
        #   -a prettified board which is within the n move range
        #   -a random prettified board generated by taking n random moves
        #making a (move, anchor, pos, negative)
        board = game.board()
        info = []
        moves = list(game.mainline_moves()) # so we can easily iterate over the game states considered
        i=0
        while i < len(moves):
            if board.is_checkmate():
                break
            if board.turn:
                ...
                turn = -1
            #GET RANDOM CORRELATED MOVE
            #IN RANGE(1,3,5)
            random_range= [1,3,5,7] #this would be the position after player makes the last move ie its the other players mnove in this state
            if len(moves) - i > 4:
                rand_num=3
            else:
                break
            ...
            return_state = board.copy(stack=False)#so we know what state to return to after generating a positive positon
            anchor = Data_Processing.format_pieces(board)
            counter= 0
            for z in range(rand_num):
                 
                board.push(moves[i+z])
                counter+=1
            #convert board into data_format
            pos = Data_Processing.format_pieces(board)
            #get negative
            mainline_moves = random.randint(0, rand_num-1)#so we can follow the mainline for n moves making us have samples of close negatives and far negatives
            i_hate_python = 0
            for k in range(mainline_moves):
                return_state.push(moves[i+k])
                i_hate_python =k
            remaining = rand_num-i_hate_python
            for z in range(remaining):#produce random moves from this point on
                #get move
    ...
    def get_random_move(board: Board) -> chess.Move:
        #returns a random legal move given a board state
    ...
    def get_pgn_to_test():
        file ='./data_file/Akobian.pgn'
        pgns = open(file)
        first_game = chess.pgn.read_game(pgns)
        print(first_game.headers["Result"])
        vals =Data_Processing.get_neighbor_triplets(first_game)
        print(len(vals))
    ...
    def make_triple_dataset():
        ...
        for file in files:
            ...
            while (game_pgn:=chess.pgn.read_game(games)) != None:
                    ...
                    try: 
                        boards = Data_Processing.get_neighbor_triplets(game_pgn)
                    except Exception as e:
                        continue
                    data = Data_Processing.Stringify_Triples(boards)
                    data = ''.join(data)
                    with open('./data_file/model_data/training_neighbors.csv', 'a+') as f:
                        f.write(data)
            games.close()
            print(f'{file} has been converted')
        print('Completed')
